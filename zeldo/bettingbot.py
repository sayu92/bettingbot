...
class Bet:
    ...
    def __init__(self, bet_interest = 0.5):
        ...
        self.compte = {}
        self.predictions = {'win' : [], 'lose' : []} # 'lose' : [list des joueurs ayant predit la defaite] 'win' : [list des joeurs pariant sur la victoire]
    ...
    def result(self, resultat ) : # Envoie d erreur si mauvais input
        ...
        for joueur in list_winners : # Calcul des gains supplementaires
            gain =math.floor(amount_to_distribut* self.compte[joueur]/amount_bet_win) + max(math.floor(self.bet_interest * self.compte[joueur]),1)
